...
1. **Threat model** â€” Identify attack surface and threats
2. **Design** â€” Plan security controls
3. **Implement** â€” Write secure code with defense in depth; see code examples below
4. **Validate** â€” Test security controls with explicit checkpoints (see below)
5. **Document** â€” Record security decisions
...
export async function getUserByEmail(email: string) {
  const { rows } = await pool.query(
    ...
    [email]  // value passed separately â€” never interpolated
  );
  return rows[0] ?? null;
}
```
...
export function validateLoginInput(raw: unknown) {
  ...
  if (!result.success) {
    // Return generic error â€” never echo raw input back
...
### Securing an Endpoint â€” Full Flow
...
app.post('/api/login', authLimiter, async (req, res) => {
  ...
  // 2. Authenticate â€” parameterized query, constant-time compare
  const user = await getUserByEmail(email);
  if (!user || !(await verifyPassword(password, user.passwordHash))) {
    // Generic message â€” do not reveal whether email exists
    return res.status(401).json({ error: 'Invalid credentials' });
  }

  // 3. Authorize â€” issue scoped, short-lived token
  ...
  // 4. Secure response â€” token in httpOnly cookie, not body
