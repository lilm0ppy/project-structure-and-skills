...
```sql
-- Running total and rank within partition â€” no self-join required
SELECT
    department_id,
    employee_id,
    salary,
...
- **Seq Scan on large table** â†’ add or fix an index
- **actual rows â‰« estimated rows** â†’ run `ANALYZE <table>` to refresh statistics
- **Buffers: shared hit** vs **read** â†’ high `read` count signals missing cache / index
