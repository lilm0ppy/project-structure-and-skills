...
1. **Assess** â€” Identify what needs monitoring (SLIs, critical paths, business metrics)
2. **Instrument** â€” Add logging, metrics, and traces to the application (see examples below)
3. **Collect** â€” Configure aggregation and storage (Prometheus scrape, log shipper, OTLP endpoint); verify data arrives before proceeding
4. **Visualize** â€” Build dashboards using RED (Rate/Errors/Duration) or USE (Utilization/Saturation/Errors) methods
5. **Alert** â€” Define threshold and anomaly alerts on critical paths; validate no false-positive flood before shipping
...
// Good â€” structured fields, includes correlation ID
logger.info({ requestId: req.id, userId: req.user.id, durationMs: elapsed }, 'order.created');

// Bad â€” string interpolation, no correlation
