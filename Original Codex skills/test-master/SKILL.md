...
1. **Define scope** â€” Identify what to test and which testing types apply
2. **Create strategy** â€” Plan the test approach across functional, performance, and security perspectives
3. **Write tests** â€” Implement tests with proper assertions (see example below)
4. **Execute** â€” Run tests and collect results
...
5. **Report** â€” Document findings with severity ratings and actionable fix recommendations
...
```js
// âœ… Good: meaningful description, specific assertion, isolated dependency
...
Apply the same structure for pytest (`def test_â€¦`, `assert result == expected`) and other frameworks.
...
- Mock external dependencies â€” never call real APIs or databases in unit tests
- Use meaningful `it('â€¦')` descriptions that read as plain-English specifications
...
- Use production data in tests â€” use fixtures or factories instead
- Create order-dependent tests â€” each test must be independently runnable
- Ignore flaky tests â€” quarantine and fix them; don't just re-run until green
- Test implementation details (internal method calls) â€” test observable behaviour
